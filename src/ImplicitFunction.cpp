...
float SignedDistanceFunction::Eval(glm::vec3 p) const {
    ...
    for(int i=1; i<size ; i++){
        distance = (pointcloud.getPlanePrecise(i).getCenter()-point).getNorm();
        if(distance < min_distance){
            min_distance = distance;
            min_plane = pointcloud.getPlanePrecise(i);
        }
    }

    /* la deuxième étape de l'algorithme consiste à calculer la distance d(z, X)
    ...
    */
    ...
    for(int i=1 ; i<size ; i++) {
        distance2 = (pointcloud.getCloudPrecise(i)-z).getNorm();
        if(distance2 < min_distance2) {
            min_distance2 = distance2;
            min_point = pointcloud.getCloudPrecise(i);
        }
    }
...
glm::vec3 SignedDistanceFunction::EvalDev(glm::vec3 p) const {
    return EvalDevFiniteDiff(p);
}

int SignedDistanceFunction::getSize() const {
    return pointcloud.getSize();
}

Cloud SignedDistanceFunction::getPointCloud() const {
    return pointcloud;
}

Cloud &SignedDistanceFunction::getPointCloud() {
    return pointcloud;
}
